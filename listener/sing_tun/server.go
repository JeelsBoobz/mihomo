...
func New(options LC.Tun, tunnel C.Tunnel, additions ...inbound.Addition) (l *Listener, err error) {
	...
	if options.AutoRedirect {
		...
		for _, routeAddressSet := range options.RouteAddressSet {
			...
			if !loaded {
				return nil, E.New("parse route-address-set: rule-set not found: ", routeAddressSet)
			}
		...
		for _, routeExcludeAddressSet := range options.RouteExcludeAddressSet {
			...
			if !loaded {
				return nil, E.New("parse route-exclude_address-set: rule-set not found: ", routeExcludeAddressSet)
			}
	...
	if l.autoRedirect != nil {
		...
		if err != nil {
			return nil, E.Cause(err, "auto redirect")
		}
